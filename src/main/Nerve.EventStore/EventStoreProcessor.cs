namespace Kostassoid.Nerve.EventStore
{
	...
	internal class EventStoreProcessor : Cell
	{
		readonly IEventStorage _storage;

		public EventStoreProcessor(IEventStorage storage) : base("EventStoreProcessor", ThreadScheduler.Factory)
		{
			_storage = storage;
		...
		{
			return this.SendFor<T>(new AggregateIdentity(typeof (T), id)).Result;
		}
